...
class SyllogismRule(Rule):
    ...
    def __init__(self, which_rule, expression, resulting_expression, neg_case=-1):
        ...
        if which_rule == 1:
            self.description = (
                "All x is A => attach object x to argument A for all cases"
            )
        if which_rule == 2:
            self.description = (
                "Some x is (not) A => introduce new object x for argument A"
            )
        if which_rule == 3:
            self.description = (
                "No x is A => attach object x to argument A for all cases"
            )
        if which_rule == 4:
    ...
    def get_explanation(self):
        ...
        return dict(
            ...
            else [expression.get_string_rep() for expression in self.expression],
            out_expression=[
                [self.resulting_expression.get_string_rep()]
                if type(self.resulting_expression) != list
                else [
                    expression.get_string_rep()
                    for expression in self.resulting_expression
                ]
            ],
        )
    ...
    def apply_rule1(clause: SyllogismExpression, *args):
        ...
        return (
            new_clauses,
            SyllogismRule(1, [clause, used_comp_clause], created_syllogism_expression),
        )
    ...
    def apply_rule3(clause: SyllogismExpression, *args):
        ...
        return (
            new_clauses,
            SyllogismRule(3, [clause, used_comp_clause], new_clauses[0])
            if used_comp_clause is not None
            else None,
        )
