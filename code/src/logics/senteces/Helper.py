...
def create_expression_representation(expression, ret_dict=None):
    ...
    if expression.negated and type(expression) != FunctionExpression:
        ret_dict["list"].append(dict(type=0, tokens=expression.negated))
    ...
    if type(expression) == SyllogismExpression:
        ...
        ret_dict["list"].append(
            dict(type=-1, name="2. Keyword", tokens=expression.syllogism_keywords[1])
        )
        ret_dict["list"].append(
            dict(type=-1, name="Subject", tokens=expression.subject)
        )
        return ret_dict
        ...
        ret_dict["list"].append(
            create_expression_representation(expression.left_expression, dict())
        )
        ret_dict["list"].append(dict(type=-2, tokens=expression.connection_keyword))
        ret_dict["list"].append(
            create_expression_representation(expression.right_expression, dict())
        )
        return ret_dict
        ...
        ret_dict[
            "name"
        ] = f"{'Left' if expression.left_match else 'Right'} Conditional Expression"
        if expression.left_match:
            ret_dict["list"].append(dict(type=-3, tokens=expression.key_words[0]))
        ret_dict["list"].append(
            create_expression_representation(expression.premise, dict())
        )
        ret_dict["list"].append(
            dict(
                type=-3, tokens=expression.key_words[1 if expression.left_match else 0]
            )
        )
        ret_dict["list"].append(
            create_expression_representation(expression.conclusion, dict())
        )
        return ret_dict
        ...
        ret_dict["list"].append(dict(type=-4, tokens=expression.quantification_split))
        ret_dict["list"].append(
            create_expression_representation(expression.quantified_expression, dict())
        )
        return ret_dict
        ...
        ret_dict["list"].append(dict(type=-5, name="Verb", tokens=expression.verb))
        ret_dict["list"].append(
            dict(type=-5, name="Object1", tokens=expression.object1)
        )
        ret_dict["list"].append(
            dict(type=-5, name="Preposition", tokens=expression.preposition)
        )
        ret_dict["list"].append(
            dict(type=-5, name="Object2", tokens=expression.object2)
        )
        return ret_dict
        ...
        ret_dict[
            "name"
        ] = f"{'F' if not expression.multi else 'Multif'}unction Expression{'*' if expression.negated else ''}"
        ret_dict["list"].append(
            dict(type=-6, name="Variable", tokens=expression.variables[0])
        )
        ret_dict["list"].append(dict(type=-6, tokens=expression.key_words[0]))
        ret_dict["list"].append(
            dict(type=-6, name="Function", tokens=expression.quantified_function)
        )
        if expression.multi:
            ret_dict["list"].append(dict(type=-6, tokens=expression.key_words[1]))
            ret_dict["list"].append(
                dict(type=-6, name="Variable", tokens=expression.variables[1])
            )
        return ret_dict
